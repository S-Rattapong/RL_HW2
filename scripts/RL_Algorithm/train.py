...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlOnPolicyRunnerCfg):
    ...
    num_of_action = 11                  # แบ่งแรงผลักรถเข็นเป็น 5 ระดับ (เช่น -2, -1, 0, 1, 2)
    action_range = [-5.0, 5.0]         # ช่วงของแรงผลัก [min, max]
    discretize_state_weight = [1, 20, 0, 0]  # น้ำหนักในการแปลง State (เน้นขยายค่ามุมไม้ให้ละเอียดขึ้น)
    learning_rate = 0.2                # ความเร็วในการเรียนรู้ (Alpha)
    n_episodes = 5000                  # จำนวนรอบที่จะให้ Agent ฝึกซ้อม
    ...
    final_epsilon = 0.05               # สุ่มน้อยที่สุดที่ 1% (เหลือเผื่อไว้กัน AI ติดหล่ม)
